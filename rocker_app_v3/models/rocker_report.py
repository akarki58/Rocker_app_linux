...
class Report(models.Model):
    ...
    collection_ids = fields.Many2many('rocker.report', 'rocker_report_collection', 'report_id', 'collection_id',
                                      'Report in Collections',
                                      domain="[('report_type', '=', 'collection'),('report_application','=', report_application)]")
    column_headings = fields.Char('Column headings', default='Report type; Count',
                                  help="Column headings separated with ;")
    ...
    _sqldriver = False
    #
    report_application = fields.Selection([('excel', 'Excel Report'), ('powerpoint', 'PowerPoint Report')],
                                          'Report App', default='excel', required=True)
    element = fields.Selection([('table', 'Table'), ('chart', 'Chart')], 'Element type', default='table',
                               required=False)
    legend = fields.Selection([('none', 'None'), ('bottom', 'Bottom'), ('right', 'Right')], 'Legend', default='bottom',
                              required=False)
    elements_per_slide = fields.Selection([('1', '1'), ('2', '2'), ('4', '4'), ('6', '6')], 'Elements per Slide',
                                          default='1', required=False)
    slide_title = fields.Char('Slide Title')
    # state = fields.Char('State', store=False, default = 'Testing')
    show_values = fields.Selection(
        [('none', 'None'), ('inside', 'Inside'), ('outside', 'Outside'), ('pros_inside', '% Inside'),
         ('pros_outside', '% Outside')], 'Show values', default='none')
    ...
    def export_ppt(self, context=None):
        ...
        if self.report_type == 'single':
            ...
            if con:
                ...
                # one per page
                slide, element_written = self._populate_pp_sql(self, engine, con, prs, slide, page_elements, pp_title,
                                                               self.select_clause, self.column_headings, cnt_report,
                                                               element_written, collection, self.lang)
            else:
            ...
            for report in self.report_ids:
                ...
                if con:
                    ...
                    slide, element_written = self._populate_pp_sql(report, engine, con, prs, slide, page_elements,
                                                                   pp_title, report.select_clause,
                                                                   report.column_headings, cnt_report, element_written,
                                                                   collection, report.lang)
                else:
                    raise exceptions.ValidationError('No DB connection')
                cnt_report += 1
        ...
        permlink = base_url + '/web/login?db=' + self.env.cr.dbname + '&redirect=' + base_url + '/web/content/rocker.report/%s/report/%s?download=true' % (
            self.id, self.file_name)
        _logger.debug('permlink: ' + permlink)
        execlink = self.env['ir.config_parameter'].sudo().get_param(
            'web.base.url') + '/web/login?db=' + self.env.cr.dbname + '&redirect=' + self.env[
    ...
    def export_xls(self, context=None):
        ...
        if self.report_type == 'single':
            ...
            if con:
                ...
                self._populate_sql(engine, con, workbook, worksheet, self.select_clause, self.column_headings,
                                   self.lang)
            else:
                raise exceptions.ValidationError('No DB connection')
        elif self.report_type == 'collection':
            ...
            for report in self.report_ids:
                ...
                if con:
                    ...
                    self._populate_sql(engine, con, workbook, worksheet, report.select_clause, report.column_headings,
                                       report.lang)
                else:
                    raise exceptions.ValidationError('No DB connection')
    ...
    def _about(self, workbook, context=None):
        ...
        aboutsheet[
            'C2'].hyperlink = "https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=DGK3E2CC42EJ4&item_name=for+Rocker+Reporting+application+development&currency_code=EUR&source=url"
        ...
        aboutsheet.cell(4, 3).value = str(fields.Datetime.context_timestamp(
            self,
            fields.Datetime.now()
        ))
    ...
    def _populate_sql(self, engine, con, workbook, worksheet, sql, headings, language, context=None):
        ...
        for head in header:
            ...
            try:
                max_row_for = max(
                    (c.row for c in worksheet[get_column_letter(worksheet.cell(row=1, column=col).column)] if
                     c.value is not None))
                _logger.debug(
                    'max row in col ' + get_column_letter(worksheet.cell(row=1, column=col).column) + ": " + str(
                        max_row_for))
            except:
                max_row_for = 0
        ...
        max_in_range = 0
        for head in header:
            max_row_for = max((c.row for c in worksheet[get_column_letter(worksheet.cell(row=1, column=col).column)] if
                               c.value is not None))
            _logger.debug('Found new max row in col ' + str(
                get_column_letter(worksheet.cell(row=1, column=col).column)) + ": " + str(max_row_for))
            if max_row_for >= max_in_range:
                max_in_range = max_row_for
            col += 1
    ...
    def _populate_pp_sql(self, report, engine, con, prs, slide, page_elements, pp_title, sql, headings, cnt_report,
                         last_element_written, collection, language, context=None):
        global chart_data
        from pptx.enum.chart import XL_CHART_TYPE
        ...
        for row in records:
            ...
            if self.element == 'table':
                if len(row) != len(header):
                    raise exceptions.ValidationError(
                        'Count of headers is not the same as count of data columns\nSeparate headers with ;\nOr check your SQL')
                if r >= rows_per_table:
                ...
                if (r == 0):
                    ...
                    if pp_title:
                        add_title = True
                    # find location
                    element_written, rows_per_table, font_size, x, y, cx, cy = self._find_place(page_elements,
                                                                                                element_written,
                                                                                                add_title)
                ...
                if self.chart_type == '15':
                    ...
                    if len(valuelist) != 3:
                        raise exceptions.ValidationError(
                            'For Bubble chart we need: Series, ValueX, ValueY, ValueSize \nWe must have 3 values (x & y)\nCheck your SQL')
                    ...
                    if len(valuelist) != 2:
                        raise exceptions.ValidationError(
                            'For XY diagram (Scatter) we need: Series, ValueX, ValueY \nWe must have 2 values (x & y)\nCheck your SQL')
                    # check if rest are numbers only
                    for item in valuelist:
                        if item is not None:
                            if not (isinstance(item, (int, float, complex)) and not isinstance(item, bool)):
                    ...
                    if self.chart_type in ['99995', '999969']:
                        ...
                        if not row[0] == prev_categoryname:
                            ...
                            raise exceptions.ValidationError(
                                'For Pie (Category from row data) every data row must contain unique Category (column 1 = Category Name)')
                    else:
                        try:
        ...
        if self.element == 'chart':
            ...
            if pp_title:
                add_title = True
            # find location
            element_written, rows_per_table, font_size, x, y, cx, cy = self._find_place(page_elements, element_written,
                                                                                        add_title)
            ...
            if self.chart_type not in ['1', '76', '77', '15', '-4120', '80', '4', '65', '66', '67', '63', '64', '-4151',
                                       '82',
                                       '81', '-4169', '75', '72', '73', '-4169', '74', '75', '72']:

                # stacked bars value only inside
                if self.chart_type in ['58', '59', '52', '53', ] and self.show_values in ['outside', 'pros_inside',
                                                                                          'pros_outside']:
                    self.show_values = 'inside'
                #
                # percentages only for pie charts
                if self.chart_type not in ['5', '99995', '69', '999969', ] and self.show_values in ['pros_inside',
                                                                                                    'pros_outside']:
    ...
    def _cron_execute_report(self):
        ...
        _process_reports = self.env.cr.execute(""" SELECT * FROM rocker_report
                     ...
                     """)
        _logger.debug(_process_reports)
        _records = self.env.cr.fetchall()
        _logger.debug(_records)
        for _report in _records:
            self = self.env['rocker.report'].search([('id', '=', _report[0])])
            _logger.info('Cron execute report: ' + self.name)
            _logger.debug(self)
            self.export_report()
            _logger.debug('Export report done')
            # email
            _logger.debug('Emailing? ' + str(self.send_by_email) + ' ' + str(self.email_to))
    ...
    def _email_report(self):
        #
        _logger.info('Sending by Email: ' + self.name)
